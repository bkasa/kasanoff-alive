...
const EXPLORATION_ID = 'career-checkup';
...
function CareerCheckupPageInner() {
    ...
    async function initAccess() {
      try {
        ...
        if (magicToken) {
          ...
          if (data.ok) {
            window.location.href = '/career-checkup';
            return;
          }
        }
  ...
  async function startConversation() {
    ...
    try {
      ...
      const historyRes = await fetch('/api/career-checkup/chat');
      if (historyRes.ok) {
      ...
      const res = await fetch('/api/career-checkup/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  function downloadTranscript() {
    ...
    const text = `Career Checkup — Conversation Transcript\n${'─'.repeat(44)}\n\n${lines.join('\n\n')}`;
    const blob = new Blob([text], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'career-checkup-transcript.txt';
    a.click();
    ...
    return (
      <div style={{ ...pageStyle, alignItems: 'center', justifyContent: 'center' }}>
        ...
        >
          ...
          >
            {justPurchased ? 'Thank you for your purchase' : 'Access your Career Checkup'}
          </h2>
          {justPurchased && (
            <p
              style={{
    ...
    <div style={pageStyle}>
      ...
      <div style={{ padding: 'clamp(60px,8vw,80px) clamp(20px,5vw,40px) 0', textAlign: 'center', flexShrink: 0, position: 'relative', zIndex: 1 }}>
        ...
        <p style={{ fontFamily: "'Source Sans 3', sans-serif", fontSize: '13px', color: C.charcoalLight, margin: 0 }}>
          A deep coaching conversation for where you are right now.
        </p>
      </div>
...
export default function CareerCheckupPage() {
  return (
    <Suspense fallback={<div style={{ minHeight: '100vh', background: '#FDF8F0' }} />}>
      <CareerCheckupPageInner />
    </Suspense>
  );
}
