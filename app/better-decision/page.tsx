...
function BetterDecisionInner() {
    ...
    async function initAccess() {
      try {
        if (magicToken) {
          ...
          if (data.ok) {
            window.location.href = '/better-decision';
            return;
          }
        }
  ...
  async function loadOrStartConversation() {
    ...
    try {
      ...
      const historyRes = await fetch('/api/better-decision/chat');
      if (historyRes.ok) {
      ...
      const res = await fetch('/api/better-decision/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          A guided conversation with an AI advisor that helps you move from confusion to clarity — and from clarity to commitment.
        </p>
      </div>
        ...
        >
          ...
          >
            {documents.length === 0 ? (
              ...
              >
                Your decision document will appear here when the conversation is complete. You can return to this page anytime to access it.
              </p>
...
export default function BetterDecisionPage() {
  return (
