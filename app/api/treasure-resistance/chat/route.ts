...
import { TREASURE_RESISTANCE_PROMPT } from '@/lib/prompts/treasure-resistance';
...
const LOCKED_MESSAGE = `Your $18 package covered one Treasure Resistance guide, with unlimited follow-ups for 90 days. That window has closed.
...
function extractDocumentText(content: string): string | null {
  const match = /\[DOCUMENT:[^\]]+\][\s\S]*?\[\/DOCUMENT\]/.exec(content);
...
export async function GET() {
  try {
    ...
    console.error('treasure-resistance GET error:', error);
    return Response.json({ messages: [] });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Determine current session kind; create a prep session if none exists yet
    ...
    const data = await callAnthropicWithRetry(
      JSON.stringify({
        ...
        max_tokens: 2048,
        system: TREASURE_RESISTANCE_PROMPT,
    ...
    // Persist document on every response that contains one
    ...
    if (isPrepComplete) {
      ...
      console.log('Treasure Resistance prep marked complete for:', email);
    }

    return Response.json({ text: cleanText, prepComplete: isPrepComplete });
  } catch (error) {
    console.error('treasure-resistance POST error:', error);
