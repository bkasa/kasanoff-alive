...
function HardConversationInner() {
    ...
    async function initAccess() {
      try {
        ...
        if (magicToken) {
          ...
          if (data.ok) {
            window.location.href = '/hard-conversation';
            return;
          }
        }
  ...
  async function enterChatSession(sessionKind: 'prep' | 'debrief') {
    ...
    setIsLoading(true);
    try {
      const historyRes = await fetch('/api/hard-conversation/chat');
      if (historyRes.ok) {
      ...
      // No history — send opener
      const res = await fetch('/api/hard-conversation/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function sendMessage() {
    ...
    try {
      ...
      // If prep just completed, transition to 'choose' on next entry
      if (data.prepComplete) {
        // Let the user read the final document; they'll re-enter to see the choose screen
      }
    } catch {
      setMessages((prev) => [
        ...prev,
  ...
  async function startDebrief() {
    try {
      const res = await fetch('/api/hard-conversation/start-debrief', { method: 'POST' });
    ...
    return (
      <div style={{ ...pageStyle, alignItems: 'center', justifyContent: 'center' }}>
        ...
        >
          ...
          >
            {doc ? `The Hard Conversation with ${doc.title.replace('The Hard Conversation with ', '')}` : 'The Hard Conversation'}
          </h2>
          <p
            style={{
              fontFamily: "'Source Sans 3', sans-serif",
          ...
          >
            Is this a follow-up to your previous conversation, or would you like to review your
            document?
          </p>
    ...
    return (
      <div style={{ ...pageStyle, alignItems: 'flex-start', justifyContent: 'center' }}>
        ...
        >
          ...
          >
            {doc?.title || 'The Hard Conversation'}
          </h1>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        @media (max-width: 700px) {
          .hc-panel-container { flex-direction: column !important; }
          .hc-doc-panel { border-left: none !important; border-top: 1px solid rgba(212,165,116,0.18) !important; max-height: 40vh !important; }
          .hc-chat-panel { flex: 1 1 auto !important; }
        }
      `}</style>
        ...
        >
          ...
          >
            {!latestDoc ? (
              ...
              >
                Your document will appear here as the conversation progresses. You can return to
                this page any time to access it.
              </p>
            ) : (
              <div
                style={{
...
export default function HardConversationPage() {
  return (
    <Suspense fallback={<div style={{ minHeight: '100vh', background: '#FDF8F0' }} />}>
      <HardConversationInner />
    </Suspense>
  );
}
